...
class UserServices {
        ...
        if (error) {
            ...
            bycrypt.compare(loginCredentials.password, loginResult[0].password, (error, result) => {
                if (error) {
                    ...
                    const token = helper.genrateToken(loginResult);
                    loginResponse = {
                        success: true,
                        statusCode: resposnsCode.SUCCESS,
    ...
    */
    getEmail = (email, callback) => {
        ...
        userModel.forgetPassword(email, (error, result) => {
            if (error) {
                ...
                const token = helper.genrateToken(result);
    ...
    /**
       ...
       * @param {*}googleInfo
       */
    loginWithGoogleAccount(googleInfo) {
        return new Promise((resolve, reject) => {
            userModel.loginWithgoogle(googleInfo).then((data) => {
                const resultData = [];
                resultData[0] = data;
                const token = helper.genrateToken(resultData);
                resolve({ data, token });
            }).catch((err) => {
                reject(err);
            });
